...
func (t *IndexMetadata) MarshalCBOR(w io.Writer) error {
	...
	if len("retrievalAuth") > 8192 {
		return xerrors.Errorf("Value in field \"retrievalAuth\" was too long")
	}

	if err := cw.WriteMajorTypeHeader(cbg.MajTextString, uint64(len("retrievalAuth"))); err != nil {
		return err
	}
	if _, err := cw.WriteString(string("retrievalAuth")); err != nil {
		return err
	}

	if len(t.RetrievalAuth) > 8192 {
		return xerrors.Errorf("Slice value in field t.RetrievalAuth was too long")
	}

	if err := cw.WriteMajorTypeHeader(cbg.MajArray, uint64(len(t.RetrievalAuth))); err != nil {
		return err
	}
	for _, v := range t.RetrievalAuth {
...
func (t *IndexMetadata) UnmarshalCBOR(r io.Reader) (err error) {
	*t = IndexMetadata{}
	...
	if extra > cbg.MaxLength {
		return fmt.Errorf("IndexMetadata: map struct too large (%d)", extra)
	}
	...
	for i := uint64(0); i < n; i++ {
		...
		case "retrievalAuth":
			...
			if extra > 8192 {
				return fmt.Errorf("t.RetrievalAuth: array too large (%d)", extra)
			}
			...
			if extra > 0 {
				t.RetrievalAuth = make([]cid.Cid, extra)
			}
			...
			for i := 0; i < int(extra); i++ {
				{
...

					{
						...
						c, err := cbg.ReadCid(cr)
						if err != nil {
							return xerrors.Errorf("failed to read cid field t.RetrievalAuth[i]: %w", err)
						}
	...
	*t = LocationArguments{}
	...
	for i := uint64(0); i < n; i++ {
		...
		case "location":
			...
			if extra > 0 {
				t.Location = make([]commands.CborURL, extra)
			}
