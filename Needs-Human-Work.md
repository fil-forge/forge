...
| [forge #14](https://github.com/fil-forge/forge/pull/14) | every subtree resynced to its upstream `main`, **rebuilt on #13** so every pull is driven through the subtree tooling. **This is the one that gates Phase 1.** | **GREEN ALL 24 on `26061659`, reviewer satisfied after four rounds — waiting only on your merge.** `itest ingot 2/3` finally completed (15:48Z) after four earlier doc-only pushes each cancelled the in-flight run, and that is the shard that runs `TestForgeMultipartManyParts`: the workflow's own `awk -v k=2 -v s=3 'NR % s == k % s'` over the sorted 14 puts it there, verified locally. So the test this PR exists to un-hide has now run and passed. `e2e` green too, which is what the guppy re-pin was for. |
