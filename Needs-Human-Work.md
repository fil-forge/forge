...
| [forge #14](https://github.com/fil-forge/forge/pull/14) | every subtree resynced to its upstream `main`, now **rebuilt on #13 and stacked on it** — every pull driven through the subtree tooling rather than resolved by hand. **This is the one that gates Phase 1.** | **OPEN on `d88f32fa`, base `claude/subtree-tool`.** Force-pushed over the old `d7f00ba9` at Petra's instruction; that head survives on `claude/subtree-resync-v2`. **Its previous sign-off is void** — the reviewer approved a head that no longer exists — so a fresh review round is running. 36 conflicts, 13 resolved by the tooling; the four defects that mattered all merged cleanly, so neither script saw them.  **`guards` went red at `d88f32fa` and is fixed in `958dc4cb`:** the `guards` JOB runs a `gofmt -s` step that is not one of the six `check-*.sh` scripts, so running those six locally passed and proved nothing. Two sprue files rewritten with `sed` had an import left unsorted within its group. |
