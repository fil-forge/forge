...
| | what | state |
|---|---|---|
| [forge #13](https://github.com/fil-forge/forge/pull/13) | **two** subtree tools now: `finish-subtree-pull.sh` (merge + the deletion audit) and `resolve-rewrite-conflicts.sh` (the module-path collisions, with the check that makes them safe) | **OPEN** on `fc20fa81`. `6f01e5e2` came back 23 of 24, the one red being the imported-code poller flake — `indexing-service#106`'s fix is ported into this PR rather than waited for (15 failures in 80 local runs before, 0 in 80 after). Eight rounds. Round eight found a `git subtree split --rejoin` silently turning a carried file into a clean audit — fixed and covered both directions. Flipped out of draft on Petra's *"unless there are major issues, I'd like to move on with it"* |
