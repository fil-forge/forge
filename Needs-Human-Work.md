...
| [forge #14](https://github.com/fil-forge/forge/pull/14) | every subtree resynced to its upstream `main`, **rebuilt on #13** so every pull is driven through the subtree tooling. **This is the one that gates Phase 1.** | **Open on `64634519`**, base back to `main` now #13 has merged. Round one of review on the rebuild found five things, all fixed: a dropped `.mockery.yaml` fix (which falsified `check-module-paths.sh`'s own header), nine pulls miscounted as ten, `SHARD_rest` under-reported as four when it is five, `d7f00ba9` claimed preserved on a branch that held a different tree, and two facts not re-ported. The hand-resolved baseline is now on `claude/subtree-resync-by-hand`. |
