...
| [forge #14](https://github.com/fil-forge/forge/pull/14) | every subtree resynced to its upstream `main`, **rebuilt on #13** so every pull is driven through the subtree tooling. **This is the one that gates Phase 1.** | **Open on `26061659`; reviewer SATISFIED after four rounds** (8 findings across rounds 1-3, all fixed). **One thing to do before merging: stop pushing and let `itest` finish.** `itest.yml` sets `cancel-in-progress`, so each of the last four doc-only pushes cancelled the previous itest run (runs 77, 79, 80, 81 all `cancelled`). The last itest SUCCESS on this branch is run 42 at `d7f00ba9` — the pre-rebuild head, with ingot at `685a6fd`. The rebuilt tree has ingot at `956fd4b`, so that green does not transfer, and `TestForgeMultipartManyParts` — the test this PR exists to un-hide — has not completed on the rebuilt lineage. Run 82 is in flight. |
