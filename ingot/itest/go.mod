// The integration suite is its own module so that ingot's does not carry a
// dependency ingot's binary never links. It boots the Forge network through
// smelt, which nothing under ingot/ outside this directory imports; keeping
// that in ingot/go.mod forced every build of ingot -- including its Docker
// build -- to resolve ../smelt before it could fetch anything.
//
// A module boundary is how Go states that. It also means these files need no
// build tag to stay out of `go build ./...`, and that CI can run them as
// their own job rather than only compiling them.
