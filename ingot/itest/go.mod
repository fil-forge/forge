// Test-only leaf module for ingot's integration suite (build tag `itest`).
//
// This is its own module so that the parent ingot LIBRARY module does not
// require smelt (an unpublished test harness): go.mod cannot distinguish
// test-only dependencies, so keeping them here keeps ingot's module graph
// consumable. Nothing imports this module, which is why replace directives
// are acceptable here and nowhere else in the repo — they never leak into a
// consumer's build.
module github.com/fil-forge/forge/ingot/itest

go 1.26.4

replace github.com/fil-forge/forge/ingot => ../
...
require (
	...
	github.com/fil-forge/libforge v0.0.0-20260727220215-5e299c46f62f // indirect
	github.com/fil-forge/ucantone v0.0.0-20260727203046-ccb77059de44 // indirect
