...
make itest-shard SHARD=encryption                       # one upstream shard (~6 min)
...
- CI runs this suite on every pull request as `itest ingot 1/3 .. 3/3`
...
- **Two sharding schemes now exist, and this repository uses its own.**
  `itest.yml` splits ingot across three runners and *derives* which tests each
  runner gets, on purpose: a hand-written list per shard is one a new test can
  fall out of. Upstream's `make itest-shard` names four shards in the Makefile
  instead. Both work here — the Makefile's `SHARD_ALL` was re-pointed at this
  repository's shape, since upstream derives it with `-tags itest ./itest` and
  itest/ is its own module here — but CI uses `itest.yml`'s.
- **The S3 compatibility report is a local-only thing.** `make s3compat`
  ...
  `fil-forge/ingot` does both from its `go-test.yml` (and publishes to GitHub
  Pages), which the consolidation dropped along with the rest of the
  per-service workflows; `itest.yml` restores the suite but not the report.
  Wiring it back up is open work.
