...
// binary read-only over the published image's binary, and each provided config
// file read-only over the service's in-container config path. binaries and
// configs map service name -> host path; piriNodes are the generated piri-N
// service names that each receive the piri binary mount. A file mounted at a
// path inside an existing directory mount shadows just that file (Docker
// resolves nested binds by path depth), so a config override coexists with the
// base compose's config-dir mount.
func RenderOverride(binaries, configs map[string]string, piriNodes []string) ([]byte, error) {
	volumes := map[string][]string{}

	addMount := func(service, hostPath, containerPath string) {
		mount := fmt.Sprintf("%s:%s:ro", hostPath, containerPath)
		if service == "piri" {
			for _, node := range piriNodes {
				volumes[node] = append(volumes[node], mount)
			}
			return
		}
		volumes[service] = append(volumes[service], mount)
	}
	...
	for service, hostPath := range configs {
		...
		if spec.configPath == "" {
			return nil, fmt.Errorf("service %q has no registered config path to override", service)
		}
		addMount(service, hostPath, spec.configPath)
	}
	...
	for name, mounts := range volumes {
		doc.Services[name] = svc{Volumes: mounts}
	}
	...
	const header = "# Auto-generated by `smelt workspace build` -- DO NOT EDIT\n" +
		"# Mounts locally-built service binaries (and any test-provided config\n" +
		"# files) over the published images.\n"
