...
byte for byte): of the 103 consumer bump events since 2026-07-01, 64 land on
the library tip; outside forge it is 41 of 56. The rest are subtree imports,
...
the Curio PDP branch merged). Four of the fourteen pin series contain SHAs not
...
minutes before the PR merged). On no day in August did the nine non-forge
consumers share a libforge pin or a ucantone pin (up to 6 distinct libforge
pins on one day; widest spread 43.0 days). Eleven straddle intervals of
breaking or additive-required library changes were open between peers that
import the changed package; seven are still open today, five of them held by
smelt (in forge, pinned at libforge 07-27 / ucantone 07-06), the others by
delegator and piri-signing-service with indexing-service, outside forge. The
RFC's "explicit, tested PR in the consuming repo" is routinely short-circuited
by pinning the library branch before it merges, and four of July's twelve
...
- **E — August divergence:** the scripted measurement
  ...
  libforge. Uptake of the one real breaking August change (`bfc05d9`,
