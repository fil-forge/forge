...
| Does the `stack` job still pass? | Pending: run dispatched on this branch after these commits; result recorded in `findings.md` "Costs measured". The Experiment A head passed `unit` (run 33796693902); its `stack` result is recorded there too. |
