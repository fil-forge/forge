...
// TestAllocate_SizeLimitBoundary pins the exact cap. 266338304 raw is the
// largest blob whose merkle tree fits in the default 256 MiB padded limit;
// one byte more spills to a 512 MiB tree, which is why the boundary matters.
func TestAllocate_SizeLimitBoundary(t *testing.T) {
	const maxRaw = 266338304
	...
	t.Run("at the limit", func(t *testing.T) {
		...
		_, err = allocs.Get(t.Context(), digest, space)
		require.NoError(t, err, "allocation recorded")
	})

	t.Run("one byte over", func(t *testing.T) {
		// Before this fix these sizes passed the handler's own check and then
		...
		require.ErrorAs(t, err, &named, "must be a named error so it becomes a receipt failure")
		require.Equal(t, BlobSizeLimitExceededErrorName, named.Name())
		require.Contains(t, err.Error(), "266338304", "message states the actual limit")

		_, err = allocs.Get(t.Context(), digest, space)
		require.Error(t, err, "no allocation recorded for a rejected blob")

		has, err := pieces.Has(t.Context(), digest)
		require.NoError(t, err)
		require.False(t, has, "no piece allocated for a rejected blob")
	})
}

// TestAllocate_UsesConfiguredPolicy proves the handler reads the policy
// rather than a baked-in constant, and that it reads through on every call so
// a runtime config change applies.
