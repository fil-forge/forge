...
func main() {
	models := []any{
		...
		queryresult.QueryResult0_1{},
	}
	const (
		cborFile = "../cbor_gen.go"
		jsonFile = "../json_gen.go"
	)
	if err := cbg.WriteMapEncodersToFile(cborFile, "queryresult", models...); err != nil {
		panic(err)
	}
	if err := jsg.WriteMapEncodersToFile(jsonFile, "queryresult", models...); err != nil {
		panic(err)
	}
	tag(cborFile)
	tag(jsonFile)
